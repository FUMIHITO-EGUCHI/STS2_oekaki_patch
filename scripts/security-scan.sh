...
#   --semgrep      semgrep: p/csharp + p/secrets（C# security + secret pattern）
...
run_semgrep() {
  ...
  if command -v semgrep >/dev/null 2>&1; then
    echo "[security-scan] semgrep: p/csharp + p/secrets (C# security + secret pattern)"
    semgrep scan --config p/csharp --config p/secrets \
      --error --severity=WARNING --severity=ERROR --metrics=off
    return
  fi
  if command -v docker >/dev/null 2>&1; then
    echo "[security-scan] semgrep (docker): p/csharp + p/secrets"
